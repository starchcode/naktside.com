...
- **`/admin`** (`app/admin/`) — a password-gated dashboard for the visit data above: total/per-source counts and a chart (via [Recharts](https://recharts.org)), filterable by date range and source. Login is a plain `ADMIN_EMAIL`/`ADMIN_PASSWORD` check (env vars, set manually — not stored anywhere else) against a signed, HTTP-only session cookie (`libs/admin-auth.js`, signed with `AUTH_SECRET`). Not linked from anywhere else on the site.
